...
parser = argparse.ArgumentParser(
    description="stack .npy files with voxelized molecules into chunks"
)
parser.add_argument("--mol_dir", type=str, required=True, help="directory with tensors")
parser.add_argument(
    "--scaffold",
    type=int,
    default=0,
...
if not SCAFFOLD:
    SAVE_DIR = Path(
        args.mol_dir.replace("tensors", "chunks")
    )  # Path("data/precomputed_chunks/rt_rdkit_rotated")
else:
...
print("numpy arrays: ", len(numpy_arrays))
if not SCAFFOLD:
    ...
    train_files = [i for i in numpy_arrays if f"SMRT_{i.stem}" in train_subset]
    test_files = [i for i in numpy_arrays if f"SMRT_{i.stem}" in test_subset]
    print("train files: ", len(train_files))
    print("test files: ", len(test_files))
...
for i in tqdm(range(chunks)):
    ...
    for f in train_files[i * N_chunk : (i + 1) * N_chunk]:
        ...
        if conformations.shape[0] != 4:
            conformations = np.tile(conformations, reps=(4, 1, 1, 1, 1))[
                :4
            ]  # copy up to 4
