...
parser.add_argument(
    "--mol_dir", type=str, help="path to directory with precomputed tensors"
)
...
for a in tqdm(arrays):
    ...
    try:
        smis = np.repeat(
            f["smile"][0].decode("utf-8"), 4
        )  # field 'smile' can be suddenly changed to int for the sake of reduce HDD usage
    except:
        smis = a.stem  # use name instead of smi, will be useful for averaging anyway
